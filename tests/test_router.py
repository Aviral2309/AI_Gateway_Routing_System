...
class TestRoutingRules:
    ...
    def test_budget_zero_forces_ollama(self):
        d = self.rules.decide(self._cls("complex", "code"),
                              ...
                              available_models=["gemini-2.5-flash", "ollama-local"])
...
class TestTracker:
    ...
    def _r(self, **kw):
        defaults = dict(
            ...
            model_used="gemini-2.5-flash", tier="simple", intent="factual",
    ...
    def test_by_model_breakdown(self):
        self.tracker.record(self._r(model_used="gemini-2.5-flash"))
        self.tracker.record(self._r(model_used="ollama-local"))
        self.tracker.record(self._r(model_used="gemini-2.5-flash"))
        m = self.tracker.get_metrics()
        assert m["by_model"]["gemini-2.5-flash"]["requests"] == 2
